...
async function checkAuth() {
  try {
    ...
    if (data.user) {
      ...
      authArea.innerHTML = `
        <button class="user-menu" onclick="handleLogout()">
          <div class="user-avatar">${initials}</div>
          <span class="user-menu-name">${data.user.full_name}</span>
          <span style="font-size:12px;color:var(--text3)">Logout</span>
        </button>
      `;
    }
  } catch (e) {
    // Server not running or error — keep default login/signup links
  }
}
